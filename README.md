...
## Порядок работы с визуалом
  **1 шаг:** блок с темы в нужной модификации примиксовывается на верхний уровень бандла;  
...
Для отображения иконок используется блок **icon**. Иконки используются в различных частях интерфейса, они могут быть как самостоятельными сущносиями или находиться внутри контролов, но чаще всего иконки используется рядом с текстовыми блоками, поэтому имеют такие же цветовые модификации, как и блок **text** и отрисовываются двух рамерах.
...
К узлам блока примиксовывается подходящий по способу представления паттерн (элементы паттерна) в нужных модификациях. Это помогает сразу получить большуя часть визуала.
